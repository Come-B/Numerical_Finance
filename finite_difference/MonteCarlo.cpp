...
MonteCarloAnti::MonteCarloAnti(BasketPayoff* payoff, BlackScholesND* target_process, BlackScholesND* target_process_anti):
    MonteCarlo(payoff,target_process),
    m_target_process_anti(target_process_anti){
}

double MonteCarloAnti::compute_payoff_fixed_number(double start_time, double end_time, size_t nb_steps, int nb_simulation){
    vector<double> res;
    double mean=0.;
    for(int i=0;i<nb_simulation/2;i++){
        m_target_process->simulate(start_time,end_time,nb_steps);
        res.push_back((*m_payoff_function)(m_target_process->spot_at_maturity()));
        mean+=res.back();
        printf("Final spots:\n%s\n",m_target_process->spot_at_maturity().to_string().c_str());
        m_target_process_anti->simulate(start_time,end_time,nb_steps);
        res.push_back((*m_payoff_function)(m_target_process_anti->spot_at_maturity()));
        mean+=res.back();
        printf("Final spots anti:\n%s\n",m_target_process_anti->spot_at_maturity().to_string().c_str());
    }
    mean /= nb_simulation;
    double var=0.;
    for(int i=0;i<nb_simulation;i++)
...
double MonteCarloAnti::compute_payoff_conf(double start_time, double end_time, size_t nb_steps, double eps, double conf, int simu_step_size){
    vector<double> res;
    double running_tot=0.;
    double running_mean=0.;
    double running_dist=0.;
